...
#Recursivly builds tree
    def __constructor_function(self, t):
        if len(t.data) > self.min_nodes:
            ...
            t.lc = self.__constructor_function(left_child)
            t.rc = self.__constructor_function(right_child)
        else:
            return t
...
#Public function that predicts value for entered row, requires root to be entered as function. built recursivly    
    def predict_value(self, row, t):
        ...
        if row[self.x_data[t.var]] <= t.value:
                return self.predict_value(row, t.lc)
        else:
...
if __name__ == '__main__':
    ...
    y = "price"
    tester_tree = DecisionTree(df, x, y, max_height= 15, min_nodes= 4)
    print(df.iloc[80])
    print(tester_tree.predict_value(df.iloc[80], tester_tree.root))
